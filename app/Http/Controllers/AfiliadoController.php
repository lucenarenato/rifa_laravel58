...
use App\Mail\ResetPasswordAfiliado;
...
class AfiliadoController extends Controller
{
    ...
    {
        $msg = null;
        return view('afiliados.email', compact('msg'));
    }
    ...
    public function recuperarSenhaStore(Request $request)
    {
        ...
        if($check){
            $inicio = Carbon::parse(date('Y-m-d H:i:s',strtotime($check->created_at)));
            $now = Carbon::parse(date('Y-m-d H:i:s', strtotime(now())));
            ...
            if($horas >= 1){
                ...
                $msg = 'Você ainda tem um Link de Recuperação de Senha válido.';
                return view('afiliados.email', compact('msg'));
            }

        }else{
            $token = Str::random(64);
            $link = route('afiliado.reset',['token'=>$token]);
            DB::table('password_resets')->insert(['email' => $request->email, 'token' => $token, 'created_at' => now()]);
            try{
                Mail::to($request->email)->send(new ResetPasswordAfiliado($link));
                $msg = 'Um Link de Recuperação de Senha foi enviado para sua caixa de email.';
            }catch(\Exception $e){
                $msg = $e->getMessage();
    ...
    public function reset()
    {
        ...
        if($check){
            ...
            if($horas >= 1){
                ...
                return view('afiliados.reset', compact('msg','status'));
            }else{
                $status = true;
                return view('afiliados.reset', compact('status'));
            }
        }else{
            $msg = 'Link Inválido!';
            $status = false;
